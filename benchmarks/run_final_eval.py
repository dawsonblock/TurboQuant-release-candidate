...
def measure_tq_attention(B, H_q, H_kv, L, D):
    ...
    cfg = TurboQuantConfig(k_bits=3, k_group_size=64, residual_topk=0, v_bits=4, v_group_size=64, mode="fast", rotation="identity")
    ...
    tq_mem = sum(getattr(comp, k).nbytes for k in ["_k_packed", "_k_scales", "_v_packed", "_v_scales", "_resid_vals", "_resid_idx"] if getattr(comp, k) is not None)
...
if __name__ == "__main__":
    ...
    print(f"Shapes: [Batch={B}, Q_Heads={H_q}, KV_Heads={H_kv}, Context={L}, HeadDim={D}]")
    print(f"Environment: {platform.platform()} | Python {sys.version.split()[0]} | MLX {mx.__version__}")
    ...
    print("\n--- Metrics ---")
    print(f"Memory:  Dense={dense_mem/1e6:.2f}MB, TQ={tq_mem/1e6:.2f}MB (Savings: {(1 - mem_ratio)*100:.1f}%)")
    print(f"Latency: Dense={dense_time*1000:.2f}ms, TQ={tq_time*1000:.2f}ms (Slowdown: {time_ratio:.2f}x)")

    print("\n--- Verdict Thresholds ---")
    print("Goal: Memory Savings > 60%  | Allowable Slowdown (pre-Metal dispatch) < 3.0x")
